...
public class TRAII_20_X6_test {

    static TRAII_20_X6 testattava = new TRAII_20_X6_tommpuur();
    ...
    public static void main(String[] args) {
        ...
        // kutsutaan N kokoisilla syÃ¶tteillÃ¤
        int nTest = 5;
        int eroYht = 0;
        boolean ok = true;
        for (int i = 0; i < nTest; i++) {
            int ero = testaa(teeTasaSyote(N, N * M * 4), print, 3);
            if (ero < 0)
                ok = false;
            eroYht += ero;
        }
        if (ok)
            System.out.println("\nEro keskimÃ¤Ã¤rin " + eroYht/nTest);
        else
        ...
        eroYht = 0;
        for (int i = 0; i < nTest; i++) {
            int ero = testaa(teeTasaSyote(N*100, N * M * 4*100), print, 3);
            if (ero < 0)
                ok = false;
            eroYht += ero;
        }
    ...
    static int testaa(Collection<Integer> syote, int print, int maxAika) {
        ...
        List<Integer> tulos1 = new ArrayList<>(syote.size() / 2 + 2);
        List<Integer> tulos2 = new ArrayList<>(syote.size() / 2 + 2);

        // mitataan myÃ¶s aika ja huomautetaan jos yliaikaa
        long alku = System.currentTimeMillis();

        testattava.ryhmitteleKahteenTasakokoiseen(syote, tulos1, tulos2, maxAika);

        long aika = System.currentTimeMillis() - alku;
        if (aika > maxAika*1100L) // 10% pelivaraa
     ...
     * @return kokonaislukukokoelma
     */
    static Collection<Integer> teeTasaSyote(int n, int sum) {
        List<Integer> L = new ArrayList<>(n);
